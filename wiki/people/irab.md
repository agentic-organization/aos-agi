# irab

**Canonical id:** `person_irab`
**Status:** active
**Primary affiliation:** [[teams/divine]]

## Role
- Contributor — keycast and divine-blossom

## Scope
Observed activity across 2 repos in the [divinevideo](https://github.com/divinevideo) GitHub organization over the last 180 days; 10 tracked contributions. Focus areas (as observed from repositories touched):

- Divine auth stack — [[projects/keycast]]
- Divine storage — [[projects/divine-blossom]]

## Repositories
- `divinevideo/divine-blossom`
- `divinevideo/keycast`

## Projects
- [[projects/divine-blossom]]
- [[projects/keycast]]

## Accounts
- GitHub: [irab](https://github.com/irab)

## History
- 2026-05-07: Page created from divinevideo GitHub discovery run (snapshot: `data/raw/github/divinevideo-2026-05-07.json`).

## Open questions
- Full name, location, timezone?
- Formal AOS affiliation (funded, contracted, independent)?
- Nostr public key, other communication channels?
- Primary maintainer responsibilities beyond repo-level contributions?
