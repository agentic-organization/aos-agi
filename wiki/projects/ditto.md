...
- 2026-05-27: Daily ingestion (`data/raw/github/daily/2026-05-27/soapbox-pub/ditto/`) captured **1 commit** by [[people/alexgleason]]: "Render embedded polls, poll votes, and other known kinds without preview" — adds explicit dispatch for kind 1068 (NIP-88 polls) and kind 1018 (poll votes) in embedded event cards, replaces the generic "not supported" tombstone with kind-labeled cards for all EXTRA_KINDS, and tightens the fallback so unknown kinds only get the tombstone. **100 events** in the 24h window.
- 2026-05-29: Daily ingestion (`data/raw/github/daily/2026-05-29/soapbox-pub/ditto/`) captured **7 commits** by [[people/alexgleason]], including releases **v2.18.1** and **v2.18.2**. Themes: BIP-21 deep-link handling — surface `sp`/`bc1` choice when a URI carries both endpoints, open the wallet Send dialog from `bitcoin:` BIP-21 deep links, and offer the same choice when scanning dual-endpoint BIP-21 URIs. UI refinements to the Send dialog: reorder to amount-then-recipient, swap Camera icon for QrCode icon, and reopen the recipient dropdown when re-tapping the input. **100 events** in the 24h window.
- 2026-05-30: Daily ingestion (`data/raw/github/daily/2026-05-30/soapbox-pub/ditto/`) shows **1 commit** by [[people/alexgleason]] on the GitHub mirror: "Replace genUserName with \"Anonymous\" fallback" — removes the deterministic adjective+noun username generator, replaces it with a plain "Anonymous" fallback for users with no name/display_name, deletes `src/lib/genUserName.ts` and its test, and updates the `NoteContent` test to assert the new fallback. **100 events** in the 24h window.
- 2026-05-31: Daily ingestion (`data/raw/github/daily/2026-05-31/soapbox-pub/ditto/`) shows **6 commits** by [[people/alexgleason]] on the GitHub mirror, including releases **v2.18.4** (2026-05-31) and **v2.18.3** (2026-05-30). Themes: Upgrade Nostrify dependency, open `nostr:` NIP-21 URIs as deep links on native builds, fail over on 404 from always-present Esplora paths, add custom fee rate to wallet Send and stop showing fake 0 sat/vB. **100 events** in the 24h window.
- 2026-06-01: Daily ingestion (`data/raw/github/daily/2026-06-01/soapbox-pub/ditto/`) shows **4 commits** by [[people/alexgleason]] on the GitHub mirror. Themes: wallet Send UX polish and feed stability. "Show Bitcoin address row above silent payment row in Send dropdown" reorders payment options. "Auto-select unambiguous recipients in Send picker" creates chips immediately for bare npub/nprofile/bitcoin addresses while keeping dual-endpoint URIs and nip05 in dropdowns. "Keep feed on screen during refetch instead of flashing empty state" adds `useStickyFeedItems` hook to retain last non-empty list across query-key churn caused by background syncs. "Remove damus.io and nos.lol from default app relays" trims default relay list. **100 events** in the 24h window.
