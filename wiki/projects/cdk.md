...
- 2026-05-29: Daily ingestion (`data/raw/github/daily/2026-05-29/cashubtc/cdk/`) shows **1 commit** by [[people/thesimplekid]]: "docs: document FFI release prerequisites" listing secrets and repository variables needed for FFI releases. **14 issues updated** with several closures: #1949 (multiple lightning backends per currency — closed), #1791 (React Native bindings — closed), #1981 (FFI improvements — closed), #1923 (RestoreOptions for Wallet::restore batching — closed), #2010 (weekly meeting agenda 2026-05-27 — closed), #2004 (remove swagger openapi support — closed). Remaining open issues include #2012 (multiple payment backends), #2000 (golang release), #1990 (uuid v7 for better indexing), #2011 (zstd-safe regtest build failure), #1982 (React Native Nitro bindings), #2001 (auth method specificity), and #1931 (offline ecash DLEQ verification).

- 2026-06-01: Daily ingestion (`data/raw/github/daily/2026-06-01/cashubtc/cdk/`) shows **4 commits** by [[people/thesimplekid]] and [[people/asmogo]]. Major feature: "feat(mintd): allow multiple lightning backends per currency unit" (#2015) by asmogo, merged by thesimplekid — enables mint operators to configure redundant lightning backends per currency. Follow-up fixes by thesimplekid: "fix: ensure backend support", "fix: multiple keyset rotations", and "fix: env var multiple units". **23 issues updated** including bitreq HTTP client (#1608), multiple backends (#2015), and ongoing FFI/release pipeline work.
